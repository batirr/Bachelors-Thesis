...
# Load the ResNet50 model pre-trained on ImageNet and exclude top layers
...
model.compile(optimizer=Adam(learning_rate=0.001), loss=focal_loss(gamma=2., alpha=.25), metrics=['accuracy'])
...
# Compile the model again for fine-tuning with focal loss
...
# Save the model
model.save('skin_lesion_classification_model_focal_loss5.h5')
